...
function SessionReview() {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            const fileName = `${activeSession.role.replace(/\s+/g, '_')}_Interview_Report.pdf`;
            pdf.save(fileName);
        } catch (err) {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8 sm:space-y-12 animate-in fade-in duration-700">
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-bold text-sm transition-all shadow-sm
                        ${isDownloading
                            ...
                            : 'bg-teal-600 text-white hover:bg-teal-400 active:scale-95'
                        }`}
                >
                    {isDownloading ? (
                        <>
            ...
            {/* ✅ Everything below here is captured in the PDF */}
            <div ref={reportRef}>
                ...
                {/* --- Chart --- */}
                <div className="bg-white p-6 sm:p-10 rounded-3xl sm:rounded-[3rem] shadow-sm border border-slate-50 mt-8">
                    <h3 className="text-[10px] font-black text-slate-400 mb-6 uppercase tracking-[0.2em]">Per-Question Performance</h3>
                    <div className="h-64 sm:h-80">
                        <Bar
                            data={barData}
