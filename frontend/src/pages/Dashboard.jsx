...
import { useSelector, useDispatch } from 'react-redux'
import { useNavigate } from 'react-router-dom'
import { createSession, getSessions, reset, deleteSession, fetchWeaknesses } from '../features/sessions/sessionSlice' // ← NEW
import { toast } from 'react-toastify'
import SessionCard from "../components/SessionCard"
import WeaknessCard from "../components/WeaknessCard" // ← NEW
...
const Dashboard = () => {
  ...
  const navigate = useNavigate();
  const { user } = useSelector((state) => state.auth);
  const { sessions, isLoading, isGenerating, isError, message, weaknesses, weaknessLoading } = useSelector((state) => state.sessions); // ← NEW
  const isProcessing = isGenerating;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-12 space-y-8 sm:space-y-12 animate-in duration-700">
      ...
      {/* ← NEW: Weakness Tracker */}
      <WeaknessCard weaknesses={weaknesses} loading={weaknessLoading} />
