...
async def generate_questions(request:QuestionResquest):
    ...
    try:
        ...
        # ✅ FIX 1: Added one line to prevent Mistral from including code blocks in question text
        system_prompt=(
            "You are a professional technical interviewer. "
            "Task: Generate interview questions. No conversational text or numbering. "
            f"Crucial : {intruction}"
            "Output exactly one question per line. "
            "IMPORTANT: Do NOT include any code blocks, backticks, or markdown formatting in the questions. "
            "For coding challenges, describe what to implement in plain English only. "
        )
        ...
        # ✅ FIX 2: Replaced naive split('\n') with a parser that strips code blocks
        raw_text = re.sub(r'```[\s\S]*?```', '', raw_text)  # remove any code blocks
        ...
        for line in raw_text.split('\n'):
            ...
            if not clean:
                continue
            if clean.startswith('`'):  # skip stray backtick lines
                continue
            clean = re.sub(r'^(Q?\d+[\.\)]\s*)', '', clean)  # remove numbering like "1." "Q1."
            if clean:
